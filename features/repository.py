...
class RepositoryHandler:
    ...
    async def show_repository_info(self, message_or_query, repo_name, context):
        """Show repository information with loading animation"""
        # Store current repo in user data
        context.user_data["current_repo"] = repo_name

        # Get the message object
        message = message_or_query if hasattr(message_or_query, 'edit_text') else message_or_query.message
        ...
        try:
            ...
            await self._show_repo_error(message, repo_name, "Fetch Error")

    async def refresh_repository(self, message_or_query, repo_name, context):
        """Refresh repository information with loading animation"""
        # Get the message object
        message = message_or_query if hasattr(message_or_query, 'edit_text') else message_or_query.message
        ...
        try:
            ...
            if not repo_data:
                await self._show_repo_error(message, repo_name, "Refresh Failed")
                return

            # Update context with fresh data
            context.user_data["current_repo"] = repo_name
            context.user_data["repo_data"] = repo_data

            # Format and send updated repository information
            repo_info = format_repo_info(repo_data)
            keyboard = self.create_repo_keyboard()
            ...
            await self._show_repo_error(message, repo_name, "Refresh Error")
